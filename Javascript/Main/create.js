...
class create {
        ...
        for (let i = 0; i < this.model_mat.matrix_val.length; i++) {
            if (this.model_mat.matrix_val[i][0] == "v") {
                this.model_vertex.push(
                    // Matrix.vec_to_mat(new Points(this.model_mat.matrix_val[i][1], this.model_mat.matrix_val[i][2], this.model_mat.matrix_val[i][3], 1)))
    ...
    updateOrigin() {
        ...
        document.addEventListener("mousedown", (e) => {
            // print(this.model_vertex.matrix_val[0][0])
            // this.model_mat.matrix_val[0][1] = 0
            // this.model_vertex.matrix_val[0][0] = 0
            this.model_vertex[0].matrix_val[0][0] = mouseX / window.innerWidth
        })
    }
    
    track() {
        drawCircle(this.origin.x, this.origin.y, 5, "red")
        if (this.hover) {
